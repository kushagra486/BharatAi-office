...
- The current 11 per-agent sheets are generated by
  `frontend/scripts/generate-agent-sprites.py` (a second, separate
  generator from the shared rig's `generate-pixel-art.py`, since these are
  standalone full-color files, not grayscale-for-tinting). It builds every
  agent from the same body rig but gives each a **distinct head
  accessory/hairstyle silhouette** — hard hat, glasses, spiky hair, beret,
  goggles, ponytail, visor, bun, cap+brim, headset, and Nova's floating
  core — plus its own hue, so agents read as different characters at a
