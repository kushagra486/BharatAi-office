...
export function AgentsWorkGraph({ agents, tasks }: AgentsWorkGraphProps) {
  ...
  return (
    <div className="rounded-2xl border border-line bg-surface p-4 shadow-elevated">
      <p className="text-[13px] font-semibold text-ink-faint">Agents work graph — tasks completed</p>
      {!anyWork ? (
        <p className="mt-3 text-sm text-ink-faint">No completed tasks yet.</p>
      ) : (
        <ul className="mt-3 space-y-2">
          {counts.map(({ agent, done }) => (
            <li key={agent.id} className="flex items-center gap-2" title={`${agent.name}: ${done} completed`}>
              <span className="w-16 shrink-0 truncate font-mono text-[11px] text-ink-muted">{agent.name}</span>
              ...
              <span className="w-5 shrink-0 text-right font-mono text-[11px] tabular-nums text-ink-muted">{done}</span>
            </li>
          ))}
        </ul>
      )}
